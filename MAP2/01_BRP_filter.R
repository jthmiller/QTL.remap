...
DROP1 <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP1M',]
DROP1 <- names(DROP1)[which(as.numeric(DROP1)==2)]
DROP2 <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP8F',]
...
bfix <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP1M',]
bfix_swit1 <- names(bfix)[which(as.numeric(bfix)==1)]
bfix <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP8F',]
...
bfix1 <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP1M',]
bfix1 <- names(bfix1)[which(as.numeric(bfix1)==3)]
bfix2 <- pull.geno(cross)[cross$pheno$ID=='BRP_BRP8F',]
bfix2 <- names(bfix2)[which(as.numeric(bfix2)==1)]
parABxAB <- intersect(bfix1,bfix2)

gt_nopar <- geno.table(subset(cross,ind=!cross$pheno$ID %in% c('BRP_BRP1M','BRP_BRP8F')))
...
cross.1 <- subset(cross.1,ind=!cross.1$pheno$ID%in%c('BRP_BRP1M','BRP_BRP8F'))
...
################################################################################
#### Pvalue and Missing ##############################################
gt <- geno.table(subset(cross, ind=!cross$pheno$ID %in% c(toss.missing,'BRP_BRP1M','BRP_BRP8F')))
bfixA <- rownames(gt[which(gt$P.value > 0.0001 & gt$missing < 5),])
################################################################################
cros.bk <- cross
###### FILTER #######################################################
cross <- pull.markers(cross,bfixA)
cross <- subset(cross,ind=!cross$pheno$ID %in% c(toss.missing,'BRP_BRP1M','BRP_BRP8F'))
################################################################################
