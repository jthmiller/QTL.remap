## Determine which individuals to keep SAMPLE 10869 in ELR is really odd
...
col_breaks = c(seq(0, 0.3, length = 30), seq(0.31, 0.41, length = 5), seq(0.42, 1, 
  length = 40))
...
### Drop Simiar ind
cross <- subset(cross, ind = (!cross$pheno$ID %in% in.drop) | is.na(cross$pheno$Pheno))
### Drop ind with greater than 50% missing data
cross <- subset(cross, ind = nmissing(cross)/sum(nmar(cross)) < 0.5 | is.na(cross$pheno$Pheno))
