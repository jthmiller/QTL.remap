...
if (confirmed == F) {
  ...
  # gt.pars <- geno.table(cross.pars) gt.pars.set2 <-
  # gt.pars[which(gt.pars$missing==1),] gt.pars.set2 <-
  # rownames(gt.pars.set2)[which(gt.pars.set2$AA==1 | gt.pars.set2$BB==1)] gt.pars
  # <- rownames(gt.pars)[which(gt.pars$AA==1 & gt.pars$BB==1)] gt.pars <-
  # c(gt.pars,gt.pars.set2)
...
#### Filter Conservative
print(paste("Dropping markers with", miss1, "or more genotypes missing"))
...
if (chrom.b4 > 1) {
  ...
  while (chrom.b4 > chrom.after) {
    ...
    cross.18 <- formLinkageGroups(cross.18, max.rf = finRf, min.lod = finLod, 
      reorgMarkers = TRUE)
...
## Pull genotypes for QTL markers gi <- pull.geno(cross.18)[,tokeep]

### Continue to keep that are linked to markers that have been previously mapped
### keep <- sapply(1:nchr(cross.18),function(i){ a <-
### sum(X==gsub('\\:.*','',markernames(cross.18,chr=i))) b <- sum(tokeep %in%
### markernames(cross.18,chr=i)) return(a+b > 2) } ) keep <-
### names(cross.18$geno)[keep] cross.18 <- subset(cross.18, chr=keep)
...
if (reorder.marks == T) {
  ...
  cross.18 <- orderMarkers(cross.18, chr = X, window = 5, use.ripple = T, error.prob = 0.05, 
    map.function = "kosambi", sex.sp = F, maxit = 1000, tol = 0.01)
  print("Estimating the initial map with high errorprob")
  POS.map.18 <- est.map(cross.18, error.prob = 0.01, map.function = "kosambi", 
    chr = X, maxit = 100)
  cross.18 <- replace.map(cross.18, POS.map.18)
} else {
  print("estimating map with markers at physical positions")
  ord <- order(as.numeric(gsub(paste(X, ":", sep = ""), "", markernames(cross.18, 
    chr = X))))
...
write.cross(cross.18, filestem = paste(popdir, "/chr", X, "_", outname, ".QTLmap", 
...
# plot.geno.2(gt.missing,wtf=T, gen.main='Markers with < 5 missing GT,
# Grandparent Confirmed markers in Green')
