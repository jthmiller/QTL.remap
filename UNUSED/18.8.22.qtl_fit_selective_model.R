...
# This is a data using selective genotyping in an F2 population. There are four
# 150-cM chromosomes, each has 11 equally spaced markers. Only consider additive
# effects in the QTL model.

## Input Data

# Data format: 1. Save the data of trait values and marker genotypes as a
# example.csv file with column names.  The first column is the trait values,
# others are corresponding marker genotypes.  Each row shows the data of an
# individual.  2. Marker genotype: 2 for AA, 1 for Aa, 0 for aa ,and NA for
...
cross.18 <- read.cross(file = file.path("~/Dropbox/QTL_Paper/DATA/tempout.csv"), 
  format = "csv", geno = c(1:3), estimate.map = FALSE)
...
try <- sapply(1:24, function(Z) {
  cross.18$geno[[as.character(Z)]]$data <<- apply(cross.18$geno[[as.character(Z)]]$data, 
    2, tr.geno)
})
...
## Give the chromosome number.  n.chrom: number of chromosomes n.chrom<-4
n.chrom <- nchr(cross.18)

## Give the marker number in chromosomes.  info.chrom: a vector of number of
## markers on each chromosome info.chrom<-c(11,11,11,11)
...
# marker.map<-vector('list',n.chrom) marker.map[[1]]<-c(rep(0.15,10))
# marker.map[[2]]<-c(rep(0.15,10)) marker.map[[3]]<-c(rep(0.15,10))
...
## Marker genotypes on each chromosome.  marker.gty[[i]]: marker genotypes in the
## ith chromosome
...
## Specify the chromosome region you would like to search for QTLs and the QTLs
## you would like to condition in in the model for the serach.  For example: If
## the detected QTLs at (1, 3, 0.01), (1, 5, 0.12) and (4, 9, 0) are used for
## conditioning in the model in the search of chromosomes 2 and 3 (one QTL at a
## time) for QTLs. Here (1, 3, 0.01) indicates the position is at 0.01 Morgan away
## from the left marker of the third marker interval in the first chromosome.  aka
## chr, marker_number, morgan_away (1, 3, 0.01)
...
## Give the positions of detected QTLs.  Note that the order is sort according to
## chromosome and marker.

# posi<-vector('list',nQTL) posi[[1]]<-c(1,3,0.01) posi[[2]]<-c(1,5,0.12)
...
## Give the search informations.  sear.chrom: the information for chromosome
...
sear.chrom <- c(8)
## Give model type model: model=1 indicates using the proposed method in Lee et
## al. (2014); model=2 indicates using the truncated model
...
for (sear.chrom in 8) {
  for (sear.mar in 1:(info.chrom[sear.chrom] - 1)) {
    ...
    for (sear.int in 1:length(mov.seq)) {
      ...
      if (model == 1) {
        result[[wi]] <- QTL.estim.p(nQTL, posi, ys, yu, D.matrix, marker.map, 
          marker.gty)
      }
      if (model == 2) {
        result[[wi]] <- QTL.estim.t(nQTL, posi, ys, tL, tR, D.matrix, marker.map, 
          marker.gty)
      }
    }
  }
